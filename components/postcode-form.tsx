...
const commonCampuses = [
  { name: "University of Melbourne", location: { lat: -37.7963, lng: 144.9614 } },
  ...
  { name: "Australian National University", location: { lat: -35.2777, lng: 149.1185 } },
]
...
export default function PostcodeForm({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Fetch postcode data
      const response = await axios.get(`https://www.candlefox.com/wp-json/api/v1/postcodes-au?q=${postcode}`)
      ...
      const postcodeLocation = {
        lat: Number.parseFloat(postcodeData.latitude),
        lng: Number.parseFloat(postcodeData.longitude),
      }
      ...
      setError(error instanceof Error ? error.message : "An error occurred while checking the postcode")
      setResult(null)
    } finally {
      setLoading(false)
    }
